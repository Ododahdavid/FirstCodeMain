...
router.post("/tutor/new/course", loggedIn, Restrict("tutor"), async (req, res, next) => {
  try {
    ...
    // assigning req.user._id from the logged in middle ware to req.user.id, so when this function is ran, it knows which tutor is creating this course
...
// i restricted this function to only logged in tutors
...
router.get("/tutor/get/courses", loggedIn, Restrict("tutor"), async (req, res, next) => {
  try {
    const tutorId = req.user._id; // user ID from the middleware
